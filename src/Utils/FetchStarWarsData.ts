...
export async function fetchSpecificPeopleData(url: string): Promise<PersonProperties> {
    try {
...
export async function getPlanetData(url: string): Promise<Pick<PlanetProperties, 'name' | 'terrain' | 'population' | 'climate' | 'url'>> {
    try {
        const response = await axios.get(url);
...
export async function getFilms(url: any): Promise<{ title: string; uid: any }[]> {
    try {
