...
import {
  fetchSpecificPeopleData,
  getFilms,
  getPlanetData
} from '../Utils/FetchStarWarsData'
...
export const PersonModal: React.FC<PersonModalProps> = props => {
  ...
  const [personsData, setPersonsData] = React.useState<PersonProperties | null>(
    null
  )
  ...
  return (
    <Modal open={open} onClose={onClose}>
      <Box
        ...
        {...(props as BoxProps)} // Spread the rest of the props to allow for additional styling, this allows for users in the future to add more styling, it is a standard from what i found
      >
        <Typography variant='h5' mb={2}>
          {personsData?.name}
        </Typography>
        <Typography mt={2}>
          <b>Gender:</b> {personsData?.gender}
        </Typography>
        <Typography>
          <b>Birth Year:</b> {personsData?.birth_year}
        </Typography>
        <Typography mt={2}>
          <b>Height:</b> {personsData?.height}cm
        </Typography>
        <Typography>
          <b>Weight:</b> {personsData?.mass}
        </Typography>
        <Typography>
          <b>Hair Color:</b> {personsData?.hair_color}
        </Typography>
        <Typography mt={2}>
          <b>Skin Color:</b> {personsData?.skin_color}cm
        </Typography>
        <Typography>
          <b>Eye Color:</b> {personsData?.eye_color}
        </Typography>
